...
def _gleam_library_impl(ctx):
    ...
    inputs_depset = depset(
        direct = input_files,
        transitive = [transitive_compiled_dirs],
    )
    ...
    compile_cmd = '"{wrapper}" "{tool}" compile-package --name={name} --target=erlang --src="$(pwd)/{src}" --out="$(pwd)/{out}" --lib="$(pwd)/_gleam_lib"'.format(
...
def _get_src_dir(ctx):
    ...
    # Look for /src/ in the path and use everything up to and including it.
    parts = path.split("/")
